...
def test_scancode_v3_license_bsd2clause(capsys):
    args = get_args('BSD-2-Clause', 'scancode-v3.2', ['test2/test-2.sh'])
    main(args)
...
def test_scancode_v3_license_x(capsys):
    args = get_args('X', 'scancode-v3.2', ['test2/test-2.sh'])
    main(args)
    captured = capsys.readouterr()
...
def test_scancode_v3_license_expand(capsys):
    args = get_args('GPL-2.0+', 'scancode-v3.2', ['test2/test-2.sh'])
    main(args)
    captured = capsys.readouterr()
...
def test_scancode_v3_good(capsys):
    args = get_args('GPL-3.0', 'scancode-v3.2', ['test2/test-2.sh'])
    main(args)
    captured = capsys.readouterr()
...
def test_scancode_v3_crholder(capsys):
    args = get_args('GPL-3.0', 'scancode-v3.2',
